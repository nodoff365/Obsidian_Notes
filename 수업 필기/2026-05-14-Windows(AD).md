...
이미지 파일을 공유 폴더에 배치
폴더 우클릭 → 속성 → 공유 → 고급 공유
→ "선택한 폴더 공유" 체크 → 권한 → Everyone 읽기 허용
```

> 공유 폴더 위치는 어디든 무관 (AD, MEM1 등)

---

**2. gpmc.msc 실행 후 GPO 생성**

```
그룹 정책 개체 우클릭 → 새로 만들기 → 이름 입력
생성된 GPO 우클릭 → 편집
```
...
```
적용할 OU 우클릭 → 기존 GPO 연결 → 만든 GPO 선택
```
